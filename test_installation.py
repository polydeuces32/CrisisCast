...
def test_imports():
 ...
 print("[OK] Data ingestion service imported")
 except Exception as e:
 print(f"[ERROR] Data ingestion service import failed: {e}")
 return False
 
 try:
 from app.services.alert_service import AlertService
 print("[OK] Alert service imported")
 except Exception as e:
 print(f"[ERROR] Alert service import failed: {e}")
 return False
...
def test_configuration():
 ...
 print(f"[OK] Supported markets: {len(settings.supported_markets)}")
 
 assert settings.forecast_horizon_days > 0, "Invalid forecast horizon"
 print(f"[OK] Forecast horizon: {settings.forecast_horizon_days} days")
 
 assert settings.volatility_window_days > 0, "Invalid volatility window"
 print(f"[OK] Volatility window: {settings.volatility_window_days} days")
...
def test_api_endpoints():
 ...
 print(f"[OK] API routes created: {len(routes)}")
...
def test_cli_tool():
 ...
 print("[OK] CLI script found")
 
 # Check if executable
 if os.access(cli_path, os.X_OK):
 print("[OK] CLI script is executable")
 else:
 print("[WARNING] CLI script is not executable")
 
 return True
 
 except Exception as e:
 print(f"[ERROR] CLI tool test failed: {e}")
 return False
